<!---
	...
	Helper class to genrate all the XML that is required to send to authorize.net API.
...
<cfcomponent>
	...
	<cffunction name="createTransactionRequest" returntype="xml" hint="Main entry point for generating all the xml">
		...
		<cfif transactionType eq "priorAuthCaptureTransaction" and not structKeyExists(options, "refTransId")>
			...
			<cfoutput>
				<createTransactionRequest xmlns="AnetApi/xml/v1/schema/AnetApiSchema.xsd">
					...
					<transactionRequest>
						...
						<cfif structKeyExists(arguments, "money") and not isNull(money)>
							<amount>#money.getAmount()#</amount>
						</cfif>
						<cfif not isNull(customer)>
							<profile>
								<customerProfileId>#customer.getCustomerProfileId()#</customerProfileId>
								<cfif not isNull(paymentProfile)>
								<paymentProfile>
					...
					<cfif structkeyexists(options, "vat")>
					<otherTax> 
						<nationalTaxAmount>0</nationalTaxAmount>
						<localTaxAmount>0</localTaxAmount>
						<alternateTaxAmount>0</alternateTaxAmount>
						<alternateTaxId>0</alternateTaxId>
						<vatTaxRate>#options.vat.rate#</vatTaxRate>
						<vatTaxAmount>#options.vat.amount#</vatTaxAmount> 
					</otherTax>
					</cfif>
		...
		<cfoutput>
			<#requestType# xmlns="AnetApi/xml/v1/schema/AnetApiSchema.xsd">
				...
				<cfif listFindNoCase("createCustomerPaymentProfileRequest,updateCustomerPaymentProfileRequest", requestType)>
					...
					<paymentProfile>
					<!--- If we have a billTo --->
					<cfif not isNull(paymentProfile.getBillTo())>
					...
					<cfif isObject(card) and not isNull(card)>
					    <payment>
					      <creditCard>
				...
				<cfif ListFindNoCase("createCustomerProfileRequest,updateCustomerProfileRequest", requestType)>
					...
					<cfif !isEmpty(customer.getCustomerProfileId())>
						...
						<cfif not isNull(paymentProfiles) && ArrayLen(paymentProfiles)>
							...
							<paymentProfiles>
								...
								<cfif not isNull(paymentProfile.getPaymentMethods())>
					...
					<cfif variables.testmode && not isNull(paymentProfiles) && ArrayLen(paymentProfiles)>
						<validationMode>testMode</validationMode>
					</cfif>
				</cfif>
			</#requestType#>
