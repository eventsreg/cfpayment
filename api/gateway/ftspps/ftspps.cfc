<cfcomponent displayname="FTS/PPS Authorize.net Emulator" extends="cfpayment.api.gateway.base" hint="FTS/PPS Authorize.net Emulator" output="false">
	...
	<cffunction name="process" output="false" access="private" returntype="any">
		...
		<!---
			Minimum Requirements for Authorize.net emulation: 
			...
			This set is specified by FTP/PPS documentation:
				x_login:			Merchant ID
				x_tran_key:			Password
				x_delim_data:		TRUE
				x_delim_char:		(optional) Response delimiter, default is comma
				x_version:			(optional) Protocol version (3.0 or 3.1) - matches variables.cfpayment.GATEWAY_VERSION
				...
				x_amount:			Amount of purchase inclusive of tax ($0 for card validation)
				x_card_num:			Cardholder's credit card number
				x_exp_date:			Cardholder's credit card expiration date as MMYY
				x_card_code:		(optional) 3-4 digit security (CVV) code
				x_address:			Cardholder's street address
				x_zip:				Cardholder zip code, 5 digits or 5+4 for US
				x_country:			(optional) Country, default is US
				x_po_num:			Required for Level 2
				x_ship_to_zip: 		Required for Level 2
				x_tax:				Required for Level 2
			
			This set is holdover from Auth.net API:
	...
	<cffunction name="addCustomer" output="false" access="private" returntype="any" hint="Add customer contact details to the request object">
		...
		<cfscript>
			...
			if (structKeyExists(arguments.options, "address") && structKeyExists(arguments.options.address, "phone"))
	...
	<cfscript>
		...
		function parseResponse(gatewayResponse) {
			...
			results = insertResult(results, response, "24", "E-Mail", "x_email_customer", "");
		...
		function insertResult(results, response, listPosition, FieldName, fieldKey, defaultValue) {
			...
			if (arrayLen(arguments.response) >= arguments.listPosition && len(arguments.response[arguments.listPosition]))
